...
/**
 * Receipt for an "event" you will generate.
...
export default function ReceiptForm({ receiptId }: { receiptId?: string }) {
  // SECTION 1 — Search
  ...
  const handleGenerateInvoice = async () => {
    ...
    // Simulate server-side Receipt generation
    setIsGenerating(true);

    // In real life:
    // const res = await fetch(`/api/work-orders/${selectedId}/Receipt`, { method: 'POST' });
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 p-6">
      <div className="max-w-6xl mx-auto">
        <header className="mb-8">
          <h1 className="text-4xl font-bold text-slate-800 mb-1">Receipt</h1>
          <p className="text-slate-600">
            Search an event, select the correct one, then generate and preview the Receipt.
          </p>
        </header>
        ...
        <section className="bg-white rounded-xl shadow-md border border-slate-200 p-6 mb-8">
          ...
          <div className="mt-4 flex items-center justify-between gap-3">
            ...
            <button
              ...
              title={!selectedId ? "Pick one event first" : "Generate Receipt"}
            >
              <FileText size={18} />
          ...
          <div className="border border-slate-200 rounded-lg bg-slate-50 overflow-hidden">
            {pdfUrl ? (
              ...
              <div className="h-[50vh] flex flex-col items-center justify-center text-slate-600">
                ...
                <p className="text-sm">
                  Pick one event and click <span className="font-semibold">Generate Receipt</span>.
                </p>
              </div>
            )}
          </div>
