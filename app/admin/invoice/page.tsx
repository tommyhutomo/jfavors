...
export default function EventsList() {
  ...
  const handleEdit = (invoiceId: string) => {
    router.push(`/admin/create-invoice?id=${invoiceId}`);
  };

  const handleDelete = (invoiceId: string) => {
    if (confirm('Are you sure you want to delete this invoice?')) {
      const updatedEvents = events.filter((e) => e.id !== invoiceId);
  ...
  const handleCreateNew = () => {
    router.push('/admin/create-invoice');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-semibold">Loading invoice...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-slate-800 mb-2">Invoice Management</h1>
            <p className="text-slate-600">Manage all your invoice in one place</p>
          </div>
          <button
        ...
        {filteredEvents.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md border border-slate-200 p-12 text-center">
            <p className="text-slate-600 mb-4">No Invoice found</p>
            <button
              onClick={handleCreateNew}
        ...
        {events.length > 0 && (
          <div className="grid grid-cols-4 gap-4 mt-8">
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <p className="text-slate-600 text-sm font-semibold mb-2">Total Invoice</p>
