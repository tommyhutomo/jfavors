...
export default function EventsList() {
  ...
  const handleEdit = (eventId: string) => {
    router.push(`/admin/create-workorder?id=${eventId}`);
  };
  ...
  const handleCreateNew = () => {
    router.push('/admin/create-workorder');
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-white to-slate-50">
        <div className="text-center">
          ...
          <p className="text-slate-600 font-semibold">Loading workorder...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-4xl font-bold text-slate-800 mb-2">Workorder Management</h1>
            <p className="text-slate-600">Manage all your workorder in one place</p>
          </div>
          <button
        ...
        {filteredEvents.length === 0 ? (
          <div className="bg-white rounded-xl shadow-md border border-slate-200 p-12 text-center">
            <p className="text-slate-600 mb-4">No workorder found</p>
            <button
              onClick={handleCreateNew}
              className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
            >
              Create First Workorder
            </button>
          </div>
        ) : (
        ...
        {events.length > 0 && (
          <div className="grid grid-cols-4 gap-4 mt-8">
            <div className="bg-white rounded-xl shadow-md border border-slate-200 p-6">
              <p className="text-slate-600 text-sm font-semibold mb-2">Total Workorder</p>
